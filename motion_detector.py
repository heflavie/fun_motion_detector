...
def detect_motion_in_frame(previous_gray, current_frame):
    ...
    for contour in contours:
        if cv2.contourArea(contour) > 500:
            ...
            cv2.rectangle(
                frame_diff_bgr, (x, y), (x + w, y + h), (0, 0, 255), 2
            )
            cv2.putText(
                frame_diff_bgr, "Motion detected!", (x, y - 10),
...
def detect_motion(video_path=None):
    ...
    Args:
        video_path (str): Optional path to a video file.
                         If not provided, the camera will be used.
    """
    ...
    logging.info(
        "Motion detection system initialized. Starting motion detection loop."
    )
    ...
    while True:
        ...
        motion_detected, frame_diff_bgr = detect_motion_in_frame(
            previous_gray,
            current_frame
        )
        ...
        if no_display and time.time() - start_time > 10:
            logging.info(
                "Exiting after 10 seconds in CI environment without display"
            )
            break
